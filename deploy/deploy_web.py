...
for root, directories, filenames in os.walk('.\\build'):
    for filename in filenames:
        f = os.path.join(root, filename)
        if 'html' in f:
            g = open(f, 'rb')
            b.put_object(Body=g.read(), Key=f.replace('\\', '/').replace('./build/',''), ContentType='text/html')
            g.close()
        elif 'css' in f:
            g = open(f, 'rb')
            b.put_object(Body=g.read(), Key=f.replace('\\', '/').replace('./build/',''), ContentType='text/css')
            g.close()
        else:
            b.upload_file(f, f.replace('\\', '/').replace('./build/',''))
